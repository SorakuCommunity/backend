...
export async function advancedSearch(options = {}) {
  const {
    query, type = 'ANIME', page = 1, perPage = 20,
    format, sort, genres, id, year, status, season, countryOfOrigin
  } = options;
  ...
  const result = await queryanilist(q, { search: query, type, page, perPage, format, sort, genres, id, year, status, season, countryOfOrigin });
  return {
    success: true,
    data: result.Page.media.map(mapAnime),
    meta: {
...
export async function getAiringSchedule(page = 1, perPage = 20, weekStart, weekEnd, notYetAired = true) {
  ...
  `;
  const result = await queryanilist(q, { page, perPage, start, end, notYetAired });
  return {
    success: true,
...
export async function getSeasonAnime(year, season) {
  ...
  `;
  const result = await queryanilist(q, { year: parseInt(year), season: season.toUpperCase(), page: 1, perPage: 20 });
  return {
    success: true,
    data: result.Page.media.map(mapAnime),
...
function mapAnimeDetail(media) {
  return {
    ...
    characters: media.characters?.nodes?.slice(0, 10).map(c => ({
      id: c.id,
      name: c.name.full || c.name.preferred,
      image: c.image.large || c.image.medium
    })) || [],
    relations: media.relations?.edges?.map(r => ({
...
export default { search, advancedSearch, getTrendingAnime, getPopularAnime, getAnimeInfo, getAiringSchedule, getGenres, getSeasonAnime };
